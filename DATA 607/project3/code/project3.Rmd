...
j1_pivot <- j1_pivot %>%
  mutate(discrete_skill = case_when(
    ...
    str_detect(discrete_skill, 'data science') ~ 'data science skills',
    ...
    str_detect(discrete_skill, '(programming|scripting|coding)') & !str_detect(skill, '(python|sql|database)') ~ 'computer programming',
...
q4 = "CREATE TABLE IF NOT EXISTS job_posting (posting_id INT primary key, company_id INT not null, job_board_id INT not null, job_title VARCHAR(75) not null, job_title_cat VARCHAR(75), job_type VARCHAR(75) not null, seniority_level VARCHAR(75) not null)"
...
What skills are the most commonly listed across all job positions? 
```{r}
...
ggplot(data = top25_skills, aes(x=reorder(discrete_skill, num_jobs), y=pct_of_jobs)) +
...
Are there differences in skills based on position/title?
```{r}
...
top10_by_cat %>%
  ...
  mutate_at(c("Data Scientist", "Data Analyst", "Data Engineer", "Other"), ~replace_na(.,"-")) %>%
  arrange(discrete_skill) %>% 
  kable(
    col.names = (c("Skill", "Data Scientist", "Data Analyst", "Data Engineer", "Other")),
    row.names = TRUE,
    caption = "Top Data Science Skills based on Category of Job Title",
    align = c("l",rep("c",4))
  ) %>%
...
top10_by_level <- combined_by_seniority %>%
  filter(!is.na(discrete_skill)) %>%
...
top10_by_level %>%
  ...
  mutate_at(c("Entry Level", "Mid Level", "Senior Level"), ~replace_na(.,"-")) %>%
  arrange(discrete_skill) %>% 
  kable(
    col.names = (c("Skill", "Entry Level", "Mid Level", "Senior Level")),
    row.names = TRUE,
    caption = "Top Data Science Skills based on Seniority Level",
    align = c("l",rep("c",3))
  ) %>%
...
top10_by_level %>%
  ...
  kable(
      col.names = (c("Skill", "Entry Level", "Mid Level", "Senior Level")),
      row.names = TRUE,
      caption = "Top 5 Data Science Skills Common Amongst Each Seniority Level",
      align = c("l",rep("c",3))
    ) %>%
    kable_material(c("striped", "condensed"))
...
```
Across the different seniority levels, we see that the top skills that are common amongst each level of seniority are: 
